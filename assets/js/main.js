...
$('#trigger').ready(function () {
	$("h1").click(function () {
		if ($("h1").hasClass("showEle")) {
			$('h1').removeClass('showEle')
			$('h1').block('showEle')
		} else {
			$('h1').addClass('showEle');
		}
...
function App({ songs }) {
	function handleChangeMusic({ isPrev = false, playListIndex = null }) {
		...
		// PASTIKAN INI BENAR:
		// 1. Perbarui Judul Lagu (music-player__subtitle)
		updateInfo(songName_elmnt, songs[songIndex].songName);

		// 2. Perbarui Nama Artis (music-player__singer-name)
...
function Playlist({ list, handleChangeMusic }) {
	...
	function updateTheProgressBar() {
		...
		if (isPlayed && currentTime === duration) {
			handleChangeMusic({});
		}
